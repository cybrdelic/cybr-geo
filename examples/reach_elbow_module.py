...
# Existing ORBIT shoe pattern; copied from B01 so the attachment does not alter
# ORBIT.  The four clearance holes are 6.6 mm diameter, consistent with M6 use.
ORBIT_X = (-46.0, 2.0)
ORBIT_Y = (-45.0, 63.0)
...
def gear_y(teeth, module, width, x, y0, z, bore=0.0, phase=0.0):
    ...
    # rotates that shaft axis to +Y while retaining exact involute flanks.
...
def socket_screw_z(x, y, z_bottom=-8.4, z_head=5.65, shaft_r=2.9, head_r=5.2, head_h=3.0):
...
def build():
    ...
    # Bench feet are intentionally removable in ORBIT.  REACH occupies the
    # mounting face, so attachment mode omits B02 feet but changes no ORBIT CAD.
    ...
    # Reuse ORBIT's calibrated material palette so the modules read as one
    # manufactured system: 0 graphite, 1 machined Al, 2 steel, 3 bronze,
    # 4 elastomer, 5 teal anodized.

    def add(name, shape, material=0, group="reach_fixed", motion="fixed", role=""):
        ...
        p = cad_part(
            ...
            analytic_normals=True, group=group, motion=motion,
            role=role or name.replace("_", " "), provenance="designed-concept",
            finish_axis=axis, finish_origin=tuple(shape.Center().toTuple()),
        )
        parts.append(p)
        return shape

    # --- ORBIT interface -------------------------------------------------
    saddle = box(76, 140, 6, (-22, 9, -5), 2.0)
    for x, y in ORBIT_PATTERN:
        saddle = saddle.cut(cylinder(5.1, 8, (x, y, -9), "Z"))
    ...
    for i, (x, y) in enumerate(ORBIT_PATTERN):
        insert = annulus_y(5.0, 3.05, y - 3.0, 6.0, x, -5.0)
        # The insert is authored along Y only as a visible transverse press-fit
        # detail; the M6 bolt still passes vertically through the saddle bore.
        add(f"R02_{i}_Steel_thread_insert", insert, 2, "reach_output", "Captive steel insert at ORBIT mounting station")
        add(f"R06_{i}_ORBIT_M6_socket_screw", socket_screw_z(x, y), 2, "reach_output", "M6-equivalent ORBIT attachment screw")

    # Moving yoke plates tie the saddle to the elbow output shaft.
    yoke_profile = [(-60, -8), (16, -8), (10, -20), (1, -38), (-13, -52),
    ...
    for side, y0 in (("L", -51.0), ("R", 61.0)):
        ...
        hub = annulus_y(16.0, 8.15, y0, 8.0, PIVOT[0], PIVOT[2])
        yoke = yoke.fuse(hub)
        add(f"R03_{side}_Moving_yoke", yoke, 0, "reach_output", "Rigid saddle-to-elbow yoke")

    # Hollow output shaft leaves a future cable/service path through the elbow.
    output_shaft = annulus_y(8.0, 4.2, -56.0, 138.0, PIVOT[0], PIVOT[2])
    add("R04_Hollow_output_shaft", output_shaft, 2, "reach_output", "Hollow elbow output shaft")
    ...
    # Bearing packs at both fixed cheeks.
    for side, y0 in (("L", -63.0), ("R", 72.0)):
        outer = annulus_y(13.0, 10.1, y0, 7.0, PIVOT[0], PIVOT[2])
        inner = annulus_y(10.0, 8.1, y0, 7.0, PIVOT[0], PIVOT[2])
        add(f"R07_{side}_Bearing_outer_race", outer, 2, "reach_fixed")
        add(f"R08_{side}_Bearing_inner_race", inner, 2, "reach_output")
    ...
    # Cross brace and elastomer cable guard between fixed clevis plates.
    ...
    # --- exposed two-stage 16:1 reducer on +Y service side ---------------
    # Stage 2: 14T intermediate pinion -> 56T output gear, center distance 35 mm.
    out_gear = gear_y(STAGE2_OUTPUT_TEETH, GEAR_MODULE, 8.0,
                      PIVOT[0], 82.0, PIVOT[2], bore=16.1, phase=math.pi / STAGE2_OUTPUT_TEETH)
    ...
    # Stage 1 is axially offset on the same intermediate shaft.
    ...
    add("R16_Intermediate_shaft", annulus_y(4.0, 0, 72.0, 34.0, INTERMEDIATE[0], INTERMEDIATE[2]),
        2, "reach_intermediate", "Compound reduction shaft")
    add("R17_Input_shaft", annulus_y(3.0, 0, 72.0, 43.0, INPUT[0], INPUT[2]),
    ...
    # Fixed shaft supports on service side.  They locate the gear train without
    # hiding the involute engagement from inspection.
    support = plate_xz([(-7, -36), (30, -36), (34, -103), (-4, -103)], 70.0, 8.0, 2.0)
    support = support.cut(cylinder(4.2, 10, (INTERMEDIATE[0], 69, INTERMEDIATE[2]), "Y"))
    support = support.cut(cylinder(3.2, 10, (INPUT[0], 69, INPUT[2]), "Y"))
    ...
    guard = guard.cut(cylinder(10.0, 6, (INPUT[0], 107, INPUT[2]), "Y"))
    add("R20_Perforated_gear_guard", guard, 0, "reach_fixed", "Open inspection guard around reduction train")

    # Motion combines the new elbow with ORBIT's own wrist/jaw kinematics.
    def motion(part, t, e):
        theta = elbow_angle(t)
