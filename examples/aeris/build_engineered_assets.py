"""Build AERIS-E1 using the normal CYBR GEO export contract."""
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--out',type=Path,default=ROOT/'build/aeris-e1'); ap.add_argument('--exports',action='store_true')
    ...
    report=validate(a); report['feature_map']=a.metadata['feature_map']; report['limits']=a.metadata['limitations']
    report['analytic_parts']=sum(p.cad is not None for p in a.parts); report['mesh_only_parts']=[p.name for p in a.parts if p.cad is None]
    report['engineering_revision']=a.metadata['engineering_revision']; report['bearing_interface']=a.metadata['bearing_interface']
    (out/'validation.json').write_text(json.dumps(report,indent=2)+'\n')
    export_glb(a,out/'aeris.glb'); export_bom(a,out)
    section=cutaway_engineered(a); save_cache(section,out/'cutaway_cache'); export_glb(section,out/'aeris_cutaway.glb')
    ...
    print('AERIS_E1_BUILD_COMPLETE',len(a.parts),sum(len(p.faces) for p in a.parts),flush=True)
