...
Rendering is not used as evidence of strength.
"""
...
def qualify():
    ...
    moving=[p for p in a.parts if p.motion=='elbow']
    ...
    grav_p=PAYLOAD_KG*G*rp
    inertia_p=PAYLOAD_KG*rp**2
    static=grav+grav_p
    It=inertia+inertia_p
    accel=It*ACCEL
    continuous=static*1.5
    repeated=(static+accel)*1.25
    momentary=static*SHOCK_G*1.25
    ...
    out_rpm=MAX_SPEED_DEG_S/360*60
    in_rpm=out_rpm*r.GEAR_RATIO
    mot_cont=continuous/(r.GEAR_RATIO*EFF)
    mot_rep=repeated/(r.GEAR_RATIO*EFF)
    mot_shock=momentary/(r.GEAR_RATIO*EFF)

    # External 6805 pair offloads overturning moment from the compact gearhead.
    ...
    # Primary 12 mm twin pedestal-leg section, conservative 24 mm depth.
    Z=2*(12.0*24.0**2/6.0)
    pedestal_stress=bearing_moment*1000/Z

    # Existing mounting bolt groups.
    ...
    lower_center=np.array([-22.,9.,-151.])
    total_mass=sum(mass(a,p,r) for p in a.parts)
    ...
    orbit_clear=(6.8-6.03)/2-math.sqrt(2)*(.05+.05)
    lower_clear=(9.0-8.03)/2-math.sqrt(2)*(.05+.05)

    # Package dimensions of authored R6 parts.
    ...
    checks=[
      ...
      check('6805 pair static force per bearing',bearing_each,r.SUPPORT_BEARING_STATIC_N,'<=','N'),
      check('6805 pair dynamic force per bearing',bearing_dynamic,r.SUPPORT_BEARING_DYNAMIC_N,'<=','N'),
      check('6805 pair L10 life',l10,100_000_000,'>=','rev'),
    ...
    return dict(model=a.name,qualified=all(c['passed'] for c in checks),manufacturing_release=False,
      ...
      drivetrain=dict(input_rpm=in_rpm,motor_continuous_nm=mot_cont,motor_repeated_nm=mot_rep,motor_shock_nm=mot_shock,bearing_pair_force_n=bearing_each,bearing_l10_rev=l10),checks=checks,
      release_blockers=['Overlay purchased CSG-20-160-2UH-LW and ECMA-C10401 STEP/drawings.','Finalize bearing fits/preload and tolerance stack from purchased bearing class.','Prototype proof-load, thermal, cable-flex and fatigue/cycle testing.'])
