...
def test_full_model_is_large_connected_concept_not_a_toy_gear_triplet():
    ...
    truth=assert_renderable(ASSEMBLY,'concept')
    assert truth['passed']
...
def test_authored_radial_clearances_are_positive_where_clearance_is_claimed():
    ...
    clearances=ASSEMBLY.metadata['clearances']
...
def test_fasteners_and_supports_span_the_interfaces_their_roles_claim():
    front=_part('PGA04_Front_ring_screw_01').bounds
    rear=_part('PGA05_Rear_ring_screw_01').bounds
    front_housing=_part('PGA01_Front_housing').bounds
    rear_housing=_part('PGA02_Rear_housing').bounds
    ring=_part('PGA03_Fixed_internal_ring').bounds
...
def test_planet_bushings_are_two_real_support_segments_per_planet():
    for i in range(1,4):
        gear=_part(f'PGA20_Planet_{i:02}')
        bf=_part(f'PGA21_Planet_bushing_front_{i:02}')
        br=_part(f'PGA22_Planet_bushing_rear_{i:02}')
        pin=_part(f'PGA17_Planet_pin_{i:02}')
...
def test_seals_have_real_housing_seats_instead_of_overlapping_solid_housing():
    out_seal=_part('PGA12_Output_seal').bounds
    in_seal=_part('PGA13_Input_seal').bounds
    front=_part('PGA01_Front_housing').bounds
    rear=_part('PGA02_Rear_housing').bounds
    assert out_seal[0,0] < front[1,0] and out_seal[1,0] > front[0,0]
    assert in_seal[0,0] < rear[1,0] and in_seal[1,0] > rear[0,0]
    assert ASSEMBLY.metadata['interfaces']['output_support'].startswith('two 18 mm OD')
    assert ASSEMBLY.metadata['interfaces']['input_support'].startswith('two 14 mm OD')


def test_metadata_does_not_claim_unperformed_load_validation():
    text=' '.join(ASSEMBLY.metadata['assumptions']).lower()
    assert 'no stress' in text
    assert 'contact' in text
    assert 'thread helices' in text
    assert ASSEMBLY.metadata['truth_intent']=='concept'
    assert ASSEMBLY.metadata['planetary_validation']['kinematics']['fixed_ring_reduction']==5.0
