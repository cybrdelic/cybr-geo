...
def portrait():
    ...
    spec=importlib.util.spec_from_file_location('portrait_recipe_test',ROOT/'examples/human_face.py')
...
def test_obj_adapter_preserves_uv_in_actual_renderer(tmp_path):
    ...
    spec=importlib.util.spec_from_file_location('portrait_renderer_test',ROOT/'tools/render_human_face.py')
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
