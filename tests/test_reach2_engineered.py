...
def built():
    r = load(RECIPE, "reach2_recipe_test")
    return r, r.build()


def test_reach2_cad_is_valid_and_complete(built):
    r, a = built
    assert a.name == "CYBR REACH-2 + ORBIT"
...
def test_orbit_interface_is_preserved(built):
    ...
    assert all(any(p.name == f"R2_02_{i}_ORBIT_M6_10p9" for p in a.parts) for i in range(4))
...
def test_engineering_qualification_passes():
    e = load(ENGINEERING, "reach2_engineering_test")
    report = e.qualify()
