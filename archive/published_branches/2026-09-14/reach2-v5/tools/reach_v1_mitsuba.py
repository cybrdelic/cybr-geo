"""Photoreal REACH-1 + ORBIT render using the verified v9 physical pipeline.

No image generation. The assembly comes from examples/reach_elbow_module.py.
Lighting is a CC0 Poly Haven Small Workshop HDRI; the finite bench uses a CC0
scanned roughness map. Mitsuba traces beauty/albedo/shading-normal AOVs and Intel
OIDN denoises the linear HDR radiance with those geometry-derived guides.
"""
...
def main():
    ...
    p.add_argument("--pose-time", type=float, default=2.5)
    ...
    v9 = load(V9, "reach_v9_pipeline")
    base = v9.load_base()
    recipe = load(RECIPE, "reach_recipe")
    assembly = base.tune_materials(recipe.build())

    # Framing is centered between the lower flange and ORBIT palm. The elbow is
    # rendered near maximum flex so the new DOF is visually unambiguous.
    target = np.array((-10., 7., -20.), dtype=float)
    az, el = math.radians(35.0), math.radians(18.5)
    focal = 70.0
    fstop = 16.0
    sensor = 36.0
    aspect = width / height
    ...
    scene = {
        ...
        # The lower REACH flange rests on a finite physical bench only for this
        # product/engineering photograph. It is not part of the assembly model.
        "bench": {
            "type": "rectangle",
            "to_world": mi.ScalarTransform4f.translate([-8, 0, -134.04])
                        @ mi.ScalarTransform4f.scale([245, 175, 1]),
            "bsdf": bench_bsdf,
        },
    }
    ...
    beauty_pfm = args.out / "REACH_v1_beauty_linear.pfm"
    albedo_pfm = args.out / "REACH_v1_albedo.pfm"
    normal_pfm = args.out / "REACH_v1_normal.pfm"
    denoised_pfm = args.out / "REACH_v1_denoised_linear.pfm"
    ...
    noisy = args.out / "REACH_v1_ORBIT_noisy.png"
    final = args.out / "REACH_v1_ORBIT_hero.png"
    base.save_png(beauty, noisy, args.exposure)
    base.save_png(denoised, final, args.exposure)

    manifest = {
        "model": "CYBR REACH-1 + ORBIT revision 3",
        "generated_imagery": False,
        "geometry_source": "examples/reach_elbow_module.py + unchanged ORBIT recipe",
        "renderer": "Mitsuba 3 path + albedo/normal-guided Intel OIDN",
        "mitsuba_version": importlib.metadata.version("mitsuba"),
        "variant": variant,
        "resolution": [width, height], "spp": args.spp,
        "max_depth": args.max_depth, "pose_time": args.pose_time,
        "elbow_angle_degrees": math.degrees(recipe.elbow_angle(args.pose_time)),
        "reduction_ratio": recipe.REDUCTION,
        "parts": len(assembly.parts), "triangles": triangle_count,
        "orbit_interface_pattern_mm": [list(p) for p in recipe.ORBIT_PATTERN],
        "camera": {"focal_length_mm": focal, "f_stop": fstop,
    ...
    (args.out / "REACH_v1_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
