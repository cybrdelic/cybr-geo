# Legacy scan reference / v9 portrait

This is the earlier scan demonstration. It is not used by the default face recipe or as an input to the original procedural model.
...
![Human face rendered through the v9 pipeline](../media/reference_scan_face/portrait.jpg)

![Profile rendered from the same anatomy](../media/reference_scan_face/profile.jpg)

![Clay view showing the actual facial geometry](../media/reference_scan_face/clay.jpg)
...
Check out `feat/procedural-human-face-v9`. From that repository root, use the normal CYBR GEO installation plus Mitsuba 3,
...
python tools/render_reference_scan_face.py --view portrait --out build/human_face \
  --size 1440x1800 --spp 512 --batch-spp 512 --depth 14
python tools/render_reference_scan_face.py --view profile --out build/human_face \
  --size 1200x1500 --spp 384 --batch-spp 384 --depth 14
python tools/render_reference_scan_face.py --view portrait --clay --out build/human_face \
  --size 960x1200 --spp 128 --batch-spp 128 --depth 14

PYTHONPATH=src python -m pytest -q tests/test_reference_scan_face.py
```
