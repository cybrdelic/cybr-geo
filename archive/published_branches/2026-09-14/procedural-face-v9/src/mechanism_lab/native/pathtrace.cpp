...
RTCDevice cybrDevice=nullptr;RTCScene cybrScene=nullptr;
void buildSceneAcceleration(){
 cybrDevice=rtcNewDevice("threads=8");
 if(!cybrDevice)throw std::runtime_error("Embree device creation failed");
 cybrScene=rtcNewScene(cybrDevice);rtcSetSceneFlags(cybrScene,RTC_SCENE_FLAG_ROBUST);
 RTCGeometry geometry=rtcNewGeometry(cybrDevice,RTC_GEOMETRY_TYPE_TRIANGLE);
 float *vertices=static_cast<float*>(rtcSetNewGeometryBuffer(geometry,RTC_BUFFER_TYPE_VERTEX,0,RTC_FORMAT_FLOAT3,12,tris.size()*3));
 unsigned *indices=static_cast<unsigned*>(rtcSetNewGeometryBuffer(geometry,RTC_BUFFER_TYPE_INDEX,0,RTC_FORMAT_UINT3,12,tris.size()));
 for(size_t i=0;i<tris.size();i++){
  V v[3]={tris[i].p,tris[i].p+tris[i].e1,tris[i].p+tris[i].e2};
  for(int j=0;j<3;j++){for(int k=0;k<3;k++)vertices[i*9+j*3+k]=v[j][k];indices[i*3+j]=i*3+j;}
 }
 rtcCommitGeometry(geometry);rtcAttachGeometry(cybrScene,geometry);rtcReleaseGeometry(geometry);rtcCommitScene(cybrScene);
 if(rtcGetDeviceError(cybrDevice)!=RTC_ERROR_NONE)throw std::runtime_error("Embree scene build failed");
}
bool acceleratedHit(const Ray&r,Hit&h,bool any=false){
 RTCRayHit query{};auto &ray=query.ray;
 ray.org_x=r.o.x;ray.org_y=r.o.y;ray.org_z=r.o.z;
 ray.dir_x=r.d.x;ray.dir_y=r.d.y;ray.dir_z=r.d.z;
 ray.tnear=EPS;ray.tfar=h.t;ray.mask=0xffffffffu;query.hit.geomID=RTC_INVALID_GEOMETRY_ID;
 if(any){rtcOccluded1(cybrScene,&ray,nullptr);return ray.tfar<0;}
 rtcIntersect1(cybrScene,&query,nullptr);
 if(query.hit.geomID==RTC_INVALID_GEOMETRY_ID)return false;
 h.t=ray.tfar;h.u=query.hit.u;h.v=query.hit.v;h.tri=query.hit.primID;h.floor=false;h.light=-1;return true;
}
#else
void buildSceneAcceleration(){order.resize(tris.size());std::iota(order.begin(),order.end(),0);nodes.reserve(tris.size()/2);build(0,order.size());}
bool acceleratedHit(const Ray&r,Hit&h,bool any=false){return meshHit(r,h,any);}
#endif
struct Light{
...
struct Material{
 V color;float metal,rough;int pattern;
 float ior=1.5f,coat=0.f,coatRough=.2f,aniso=0.f,rotation=0.f;
 V grainAxis{1,0,0},grainOrigin{0,0,0};
};
...
bool occluded(const Ray&r,float dist){
 ...
 Hit h;h.t=dist;return acceleratedHit(r,h,true);
}
struct Frame{
 V t,b,n;
